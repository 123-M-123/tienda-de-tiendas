...
export default async function AnalyticsPage({ searchParams }: { searchParams: { vendedor?: string } }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-black shadow-xl overflow-hidden">
        ...
        <div className="p-6 space-y-5 bg-[#faf7ed]">
          {ciudades.slice(0, 5).map((city: any, i: number) => (
            <div key={i}>
              ...
              <div className="w-full h-2.5 bg-slate-200 border border-black/10 rounded-full overflow-hidden">
                <div style={{ width: `${(city.count / (ciudades[0]?.count || 1)) * 100}%` }} className="h-full bg-slate-800" />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-10">
          ...
          <div className="p-6 space-y-4">
            {canales.map((c: any, i: number) => (
              <div key={i} className="border-b border-black/10 pb-3 last:border-0">
                <div className="flex justify-between text-[10px] font-black uppercase mb-1 text-black">
                  <span>{translateChannel(c.name)}</span>
                  <span>{c.count}</span>
                </div>
        ...
        <div className="space-y-4">
          ...
          <StatCardSmall title="Tasa de Interés" value={`${avgEngagement}%`} desc="Gente que interactúa y no se va." icon={<MousePointer2 size={20}/>} />
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-4">
                {dispositivos.map((d:any, i:number) => (
                  <p key={i} className="text-lg font-black text-black">{d.name === 'mobile' ? '📱' : '💻'} {((d.count / (totalVisitas || 1)) * 100).toFixed(0)}%</p>
                ))}
              </div>
            </div>
          </div>
