...
export default function Header() {
  ...
  // 🚩 SOLO EL ADMIN ES MARCOSMARTI1980
  const isAdminReal = vendedor === "marcosmarti1980@gmail.com" || (!vendedor && pathname.includes('/panel'));

  useEffect(() => {
    // 🪄 Solo cargamos categorías si hay un vendedor real (Cliente)
    if (vendedor && !isAdminReal) {
      fetch(`/api/categorias?vendedor=${vendedor}`)
        .then(res => res.json())
        .then(data => { if (Array.isArray(data)) setDynamicCats(data) })
        .catch(() => console.log("Categorías no disponibles"));
    }
  }, [vendedor, isAdminReal]);
  ...
  return (
    <>
      <header className="sticky top-0 z-60 bg-black border-b border-white/10 shadow-2xl font-sans text-white">
        ...
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-center">
          <Link href={getHref('/')}>
            <div style={{ height: '70px' }} className="relative flex items-center transition-all">
              <img src="/logo-nuevo.png" alt="Tienda de Tiendas" className="h-full w-auto object-contain" />
            </div>
          </Link>
        </div>
        ...
        <div className="border-t border-white/5 bg-neutral-950">
          <div className="max-w-5xl mx-auto px-3 py-2 flex items-center justify-between">
            <nav className="flex items-center gap-1">
              ...
              {/* 🚩 SOLO INVITADO SI NO HAY VENDEDOR */}
              {!vendedor && !pathname.includes('/panel') && (
                <Link href="/panel/ajustes" className="px-4 py-2 bg-white/10 rounded-full text-[11px] font-black uppercase text-white">Invitado</Link>
              )}
            </nav>

            <div className="flex items-center gap-2">
              <Link href="/#contacto" className="border border-red-600 px-5 py-2 text-[11px] font-black uppercase text-white rounded-full hover:bg-white hover:text-black transition-all">Empezar</Link>
        ...
        <div className={`overflow-hidden transition-all duration-500 bg-white/5 backdrop-blur-xl ${isRow2Open ? 'max-h-20 border-t border-white/5' : 'max-h-0'}`}>
          <nav className="max-w-5xl mx-auto px-3 py-4 flex justify-center gap-6">
            {infoRow.map((item) => (
              <Link key={item.label} href={getHref(item.href)} className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 hover:text-red-600 transition-colors">{item.label}</Link>
            ))}
          </nav>
        </div>
      </header>
      ...
      <div className={`fixed inset-0 z-70 transition-all duration-500 ${isSidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <aside className={`absolute top-0 left-0 h-full w-full max-w-[320px] bg-neutral-950 border-r border-white/10 flex flex-col transition-transform duration-500 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="p-6 flex justify-between items-center border-b border-white/5">
            <span className="text-xs font-black uppercase tracking-widest text-red-600">SaaS Identity</span>
            <button onClick={() => setIsSidebarOpen(false)} className="text-white/50"><X size={24} /></button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
            <nav className="flex flex-col gap-1">
              ...
              <div className="flex flex-col">
                <button onClick={() => setShowPanelOptions(!showPanelOptions)} className={`flex items-center justify-between p-4 rounded-xl transition-all uppercase text-xs font-bold ${showPanelOptions ? 'bg-white/10 text-white' : 'text-white/80 hover:bg-white/5'}`}>
                  ...
                  <ChevronDown size={16} className={showPanelOptions ? 'rotate-180' : ''} />
                </button>
                {showPanelOptions && (
                  <div className="flex flex-col gap-1 mt-1 ml-4 border-l border-white/10 pl-2">
                    {panelSubItems.map((item) => (
                      <Link key={item.href} href={getHref(item.href)} onClick={() => setIsSidebarOpen(false)} className="flex items-center justify-between p-3 rounded-lg hover:bg-white/5 group">
                        <div className="flex items-center gap-3"><div className={item.color}>{item.icon}</div><span className="text-[11px] font-black text-white uppercase">{item.label}</span></div>
                      </Link>
                    ))}
                  </div>
                )}
            ...
            {/* 🚩 CATEGORÍAS SOLO SI ES CLIENTE REAL */}
            {!isAdminReal && dynamicCats.length > 0 && (
              <nav className="flex flex-col gap-1 border-t border-white/5 pt-6">
                <p className="text-[9px] font-black text-white/20 uppercase mb-2 ml-4">Categorías de Tienda</p>
                {dynamicCats.map((cat) => (
                  <Link key={cat.slug} href={getHref(`/categoria/${cat.slug}`)} onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/5 text-white/80 group">
                    <div className="w-6 h-6 shrink-0 opacity-40"><img src={`/icons/${cat.slug}.png`} alt="" onError={(e) => (e.currentTarget.style.display = 'none')} /></div>
                    <span className="text-xs font-bold uppercase">{cat.label}</span>
                    <ChevronRight size={14} className="ml-auto opacity-10" />
                  </Link>
                ))}
              </nav>
            )}
          </div>
          <div className="p-6 border-t border-white/5 text-center text-[9px] font-black text-slate-600 uppercase">Version 2.0 — High Performance</div>
        </aside>
      </div>
    </>
  )
